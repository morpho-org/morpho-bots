...
When adding another boundary file under `src/` or `scripts/`, rely on
`discoverJSDocSourceFiles(packageRoot)` for checker coverage and add the file to `typedoc.json` in the
same change. Do not extend the checker's legacy explicit list: `run()` replaces that list with the
dynamic discovery result before inspection. Completion criterion: the command's printed inventory
contains every added public callable exactly once.
