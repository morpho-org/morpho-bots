...
import { revertReason as defaultRevertReason, isExecutionRevert, TxSendError } from '../tx-error';
...
export function createPendingQueue({
  ...
  function latchNonceHole(nonce: number): void {
    if (nonceHoleLow === null) {
      ...
      if (nonce < nonceHoleLow) {
        nonceHoleLow = nonce;
      }
      if (nonce > nonceHoleHigh) {
        nonceHoleHigh = nonce;
      }
    }
  }
  ...
  async function reconcile(): Promise<void> {
    if (!getConsumedNonce || pending.size === 0) {
      return;
    }
  ...
  async function clearNonceHoleIfFilled(): Promise<void> {
    if (nonceHoleLow === null || !getConsumedNonce) {
      return;
    }
    const count = await tryCatch(getConsumedNonce());
    if (count.error) {
    ...
    if (count.data > nonceHoleHigh) {
      clearNonceHole('consumed', { consumed: count.data });
    }
  }
  ...
  async function reconcileOnCadence(): Promise<void> {
    blocksSeen += 1;
    if (blocksSeen % reconcileEveryBlocks === 0) {
      await reconcile();
    }
  }
  ...
  function pruneSettledCooldowns(blockNumber: bigint): void {
    for (const [label, settledBlock] of settledAt) {
      if (blockNumber - settledBlock > settledCooldownBlocks) {
        settledAt.delete(label);
      }
    }
  }
