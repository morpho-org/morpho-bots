...
// Bounds the blast radius of a single failed HTTP batch (viem's own default is 1000).
 ...
 * `batch: { wait: 0 }`), on the primary and the fallback alike, capped at {@link BATCH_SIZE} calls.
 * Opt in for a tick that fans out many small reads — an SDK fetcher issuing dozens of `eth_call`s per
 * vault; it buys nothing for a bot whose reads already go through a batch lens. Caveat: a batch shares
 * one HTTP request, so a transport-level failure fails every call in it together.
 */
export function createHttpTransport(
  ...
  const rpc = (url: string) =>
    http(url, {
      ...
      ...(options?.batch ? { batch: { wait: 0, batchSize: BATCH_SIZE } } : {})
    })
