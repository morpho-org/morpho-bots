 ...
 * `minLevel` are dropped. ALL levels write to stderr: diagnostics live on one stream so log
 * capture (the optional Vector side-car tails a spool fed from stderr alone) can never silently
 * miss a level, and stdout stays reserved for program output.
 */
