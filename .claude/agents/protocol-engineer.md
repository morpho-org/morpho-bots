...
description: >
  ...
  writeContract, simulateContract), ABI imports from @repo/contracts, token operations (approvals,
...
2. **Read only the relevant ABIs.** This repo has no hand-written ABI files. `@repo/contracts`
   exports exactly two, generated by `soltag` from `packages/contracts/solidity/interfaces/*.sol`:
   - `MorphoAbi` — from `IMorpho.sol`
   - `MidnightAbi` — from `IMidnight.sol`

   Everything else comes from the installed SDKs — `@morpho-org/blue-sdk` and
   `@morpho-org/blue-sdk-viem` for Blue and vault entities (the reallocation bots use these
   exclusively), and `@morpho-org/midnight-sdk` for Midnight. Read the installed package's real
   exports before importing; never invent one. After changing a `.sol` interface, run
   `pnpm --filter @repo/contracts run generate` and rebuild — soltag degrades to an empty ABI
...
Bots live under `bots/` (e.g. `bots/blue-liquidation`, `bots/midnight-liquidation`); the shared
runtime they build on lives under `packages/` (`@repo/bot-kit`, `@repo/swaps`, `@repo/contracts`).
Each bot lists its own protocol focus in its `README.md`.
