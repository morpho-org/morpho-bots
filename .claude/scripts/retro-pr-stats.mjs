...
function fetchPRStats(prNumber) {
  try {
    const raw = execFileSync(
      "gh",
      ["pr", "view", String(prNumber), "--repo", REPO, "--json", "number,title,additions,deletions,changedFiles,mergedAt,url"],
...
function main() {
  ...
  console.log(`| Net change          | ${totals.netChange >= 0 ? "+" : ""}${formatNumber(totals.netChange)} lines |`);
  console.log(
