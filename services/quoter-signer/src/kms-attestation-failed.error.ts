 ...
 * set, a `GetPublicKey` response without key material, a response without the resolved key ARN
 * that later `Sign` calls must pin (so a repointed alias cannot route signing to an unattested
 * key), an SPKI encoding outside the canonical
