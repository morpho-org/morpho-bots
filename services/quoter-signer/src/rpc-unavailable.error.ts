 ...
 * `middleware.read_failed` operations of this build. All are read-only: a failed read produces no
 * signing side effect, so the denial is safely retryable.
 */
export type RpcReadOperation = 'chain-id' | 'pending-nonce' | 'latest-nonce' | 'allowance'
