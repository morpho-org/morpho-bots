...
// Full-process lifecycle: spawn `morpho-queued serve --dry-run` and assert socket bind, SIGTERM → exit 0 +
...
function spawnDaemon(extraEnv: Record<string, string> = {}): Subprocess {
  const proc = Bun.spawn([process.execPath, MAIN, 'serve', '--chain', CHAIN, '--dry-run'], {
    cwd: CWD,
    env: {
      ...process.env,
      MORPHO_BOTS_HOME: home,
      RPC_URL: rpcUrl,
      LIQUIDATOR_ADDRESS: `0x${'11'.repeat(20)}`,
      ...extraEnv
    },
    stdout: 'pipe',
    stderr: 'pipe'
...
describe('morpho-queued process lifecycle', () => {
  ...
  it('appends a would_submit outcome to outcomes.jsonl on a dry-run ingest', async () => {
    ...
    expect(res.status).toBe('would_submit')

    const outcomes = readFileSync(join(home, 'queued', `outcomes-${CHAIN}.jsonl`), 'utf8')
      .trim()
      .split('\n')
      .map(line => JSON.parse(line))
    expect(outcomes.at(-1)).toMatchObject({ kind: 'queue', status: 'would_submit' })
