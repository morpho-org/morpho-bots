...
| `guardrail.cross-book-cleared` | Cross-book clearance repriced rungs (per side, count > 0). `bookClearedRungs` is the subset moved by the opposing book rather than the own bootstrap buy | `workflow`, `marketId`, `side`, `clearedRungs`, `bookClearedRungs`                                                                                                                                                                                                     |
...
The best opposing rates are part of the market state each cycle reads, so the book is an input to
the `rest` / `resize` / `recenter` decision alongside the reference rate and fresh capacities. A book
that starts constraining a rung, stops constraining it, or moves by a whole basis point while it
constrains one changes the desired quote and reconciles; a book that does none of those rests. The
clearance is applied twice, once approximately in rate space so it participates in that decision, and
once exactly in tick space at publication. `guardrail.cross-book-cleared` reports both, with
`bookClearedRungs` separating the rungs the book moved from the rungs the own bootstrap buy moved.
