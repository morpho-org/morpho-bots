...
Build and run the operator image locally with Compose. Configuration is YAML-first: Compose mounts
`quoter-bot.yaml` read-only (failing loud when it is missing) and passes through any environment
variable you export on top — a set variable always overrides its YAML counterpart, and the combined
monitor runs with a persistent `/state` volume:

```sh
cd bots/quoter-bot
cp quoter-bot.example.yaml quoter-bot.yaml
# Edit quoter-bot.yaml (chmod 600), then:
docker compose up --build
```

To only build the operator image, run from the repository root — the build context must be the
repository root so workspace packages resolve:

```sh
docker build -f bots/quoter-bot/Dockerfile.release -t quoter-bot .
```

Plain `docker run` usage (env-file and YAML variants), the keystore/AWS signer passthrough, and the
Docker Hub release pipeline are documented in the
[operations reference](./docs/reference.md#docker).
...
- **npm support** — publish a versioned npm package so the bot runs without cloning this
  repository. (Prebuilt container images already ship to Docker Hub on every release — see the
  [operations reference](./docs/reference.md#docker).)
