...
| `QUOTER_SIGNER_LAMBDA_ARN`       | `identity.quoterSignerLambdaArn`    | Quoter-signer KMS policy middleware Lambda function or alias ARN (TIB-2026-08-12); the AWS region is derived from the ARN. `--middleware` selects this backend. Fail-closed today: middleware mode exposes no generic signer, so write flows halt until the intent ports ship. |
