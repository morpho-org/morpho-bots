...
> [!WARNING]
> The quoter bot is a work in progress, must not be used yet, and is expected to be ready for use around August 12, 2026.
...
```sh
bun run --filter @morpho-org/quoter-bot playground:build
```
...
```sh
bun run --filter @morpho-org/quoter-bot start -- --version
```
...
bun run --filter @morpho-org/quoter-bot start -- --config ./quoter-bot.yml setup-check
