...
The local Compose service uses the same `/state` ownership path through a named volume. It reads
configuration from a `quoter-bot.yaml` mounted next to the compose file and passes an environment
variable into the container only when the invoking shell sets it, so any set variable overrides its
YAML counterpart; see [docs/reference.md](./docs/reference.md#docker) for the full operator
container reference.
