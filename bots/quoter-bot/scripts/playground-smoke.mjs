...
try {
  ...
  let servedRoot = dist
  if (basePath !== '/') {
    servedRoot = await createOwnedTempDirectory('quoter-bot-playground-site-')
  ...
  const userDataDir = await createOwnedTempDirectory('quoter-bot-playground-')
  const screenshotDirectory = await createOwnedTempDirectory('quoter-bot-playground-screenshots-')
