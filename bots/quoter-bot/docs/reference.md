...
Keep the file outside the repository tree (like `/etc/quoter-bot.env` below): it holds the
...
```sh
cd bots/quoter-bot
cp quoter-bot.example.yaml quoter-bot.yaml   # then edit values; chmod 600
docker compose up --build --detach
docker compose logs --follow
```

- The compose file bind-mounts `./quoter-bot.yaml` read-only and fails loud when it is missing.
...
- For an encrypted keystore, set `KEYSTORE_HOST_PATH` to the host file and set `KEYSTORE_PATH` to
  `/run/secrets/quoter-bot-keystore.json`; keep that host file outside the repository tree, or
