...
- **State volume.** Losing `/state` makes previously bot-issued offer groups unknown, which
  ...
  defaulting to `true`. A kept claim survives uninstall unmanaged; a same-name reinstall (and
  every later upgrade) detects it with a name-scoped lookup and mounts it instead of
  attempting a conflicting create. Chain truth wins for everything else on restart.
