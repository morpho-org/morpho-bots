  ...
  test('metafile allowlist keeps the browser graph off runtime, providers, secrets, logging, and observability', async () => {
    ...
    expect(
      firstParty.every(path =>
        /playground\/(?:app|model|playground-error\.utils|field-visibility\.utils|(?:collection-import|collection-validation|fragment-codec|playground-initialization|preview-generation|strict-json)\.error)\.tsx?$|src\/config\/(?:market-collections|config-validation\.error)\.ts$|src\/domain\/(?:bootstrap|ladder)\/|src\/domain\/(?:bytes32|cross-book)\.ts$|packages\/utils\//.test(
          path
        )
      ),
      firstParty.join('\n')
