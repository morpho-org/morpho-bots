...
describe('quoter-bot playground React architecture contract', () => {
  test('mounts a real React root and contains no imperative DOM controller', async () => {
    const app = await read('bots/quoter-bot/playground/app.tsx')
  ...
  test('declares cataloged React and TanStack dependencies and a TSX shell entry', async () => {
    ...
    const botPackage = JSON.parse(await read('bots/quoter-bot/package.json'))
    const html = await read('bots/quoter-bot/playground/index.html')
