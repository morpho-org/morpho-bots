...
const runEntrypoint = async (argv: readonly string[]) => {
  const process = Bun.spawn([Bun.which('bun') ?? 'bun', 'bots/quoter-bot/src/index.ts', ...argv], {
    cwd: `${import.meta.dir}/../../../../..`,
...
describe('Cli', () => {
  ...
  test('entrypoint --version succeeds without loading runtime setup environment', async () => {
    const process = Bun.spawn(
      [Bun.which('bun') ?? 'bun', 'bots/quoter-bot/src/index.ts', '--version'],
      {
        cwd: `${import.meta.dir}/../../../../..`,
        env: { PATH: Bun.env.PATH },
        stdout: 'pipe',
  ...
  test('entrypoint --json --version emits a valid JSON string', async () => {
    const process = Bun.spawn(
      [Bun.which('bun') ?? 'bun', 'bots/quoter-bot/src/index.ts', '--json', '--version'],
      {
        cwd: `${import.meta.dir}/../../../../..`,
        env: { PATH: Bun.env.PATH },
        stdout: 'pipe',
  ...
  test('entrypoint setup-check never emits provider URL credentials or nested request text', async () => {
    ...
    const process = Bun.spawn(
      [Bun.which('bun') ?? 'bun', 'bots/quoter-bot/src/index.ts', 'setup-check'],
      {
        cwd: `${import.meta.dir}/../../../../..`,
        env: {
          PATH: Bun.env.PATH,
