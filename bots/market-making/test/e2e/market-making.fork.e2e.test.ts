...
describe('market-making workflow on a pinned Base fork', () => {
  ...
  beforeAll(async () => {
    stateDirectory = await mkdtemp(join(tmpdir(), 'market-making-e2e-state-'))
    process.env.XDG_STATE_HOME = stateDirectory
    setSystemTime(new Date(Number(PINNED_WALL_TIMESTAMP) * 1_000))
    ...
    api = startRouterApi(anvil.rpcUrl)
