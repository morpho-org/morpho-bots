...
const temporaryDirectory = async () => {
  const directory = await mkdtemp(join(tmpdir(), 'market-making-config-'))
...
`)
  ...
  test('discovers .yaml first when both default filenames exist', async () => {
    const directory = await temporaryDirectory()
    await writeFile(join(directory, 'market-making.yaml'), yaml())
    await writeFile(
      join(directory, 'market-making.yml'),
  ...
  test('falls back to market-making.yml when market-making.yaml is absent', async () => {
    const directory = await temporaryDirectory()
    await writeFile(
      join(directory, 'market-making.yml'),
      yaml().replace('https://rpc.yaml.example', 'https://rpc.fallback.example')
  ...
  test('a discovered market-making.yaml FIFO fails closed without falling back', async () => {
    const directory = await temporaryDirectory()
    await createFifo(join(directory, 'market-making.yaml'))
    await writeFile(join(directory, 'market-making.yml'), yaml())
  ...
  test('rejects explicit and discovered YAML symlinks to an existing regular file', async () => {
    ...
    const discoveredLink = join(directory, 'market-making.yaml')
  ...
  test('rejects a dangling discovered YAML symlink without .yml or env-only fallback', async () => {
    const directory = await temporaryDirectory()
    await symlink(join(directory, 'missing-target.yaml'), join(directory, 'market-making.yaml'))
    await writeFile(join(directory, 'market-making.yml'), yaml())
  ...
  test.each([
    ...
    async (_operation, handle) => {
      ...
      expect(error).toBeInstanceOf(ConfigFileError)
      expect(error.reason).toBe('unreadable-discovered')
      expect(opened).toEqual(['/virtual/market-making.yaml'])
      expect(closed).toBe(true)
    }
  )
