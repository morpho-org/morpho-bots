 ...
 * @param parameters - Complete maker groups, ladder-owned sell IDs, and current block timestamp.
 * @returns The complete indexed book, with positive remaining size and tick-derived current rate
 * attached only to eligible ladder sells whose maturity evidence is valid.
 * @remarks Unknown, pending, matured, or malformed sells remain in the book without eligibility so
 * any crossing they cause still fails closed in `resolveBootstrapProspectiveOffer`.
 */
export const bootstrapLadderSellOverlapBookOffers = (parameters: {
  ...
  bootstrapBookOffers(parameters.groups).map(offer => {
    if (
      offer.buy ||
