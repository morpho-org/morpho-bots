...
import type { CooldownStore, Logger, SimulateResult } from '@repo/bot-kit';
  ...
  return async (pairs: LensInput[]) => {
    ...
    if (out) {
      for (const pair of pairs) {
        map.set(lensKey(marketId(pair.params), pair.borrower), out);
      }
    }
    return map;
  };
...
function runWith(opts: {
  ...
  if (opts.seedBackoffAt !== undefined) {
    backoff.record(LABEL, opts.seedBackoffAt);
  }
