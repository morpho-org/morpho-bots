...
async function ensureService(name: string): Promise<void> {
  ...
  if (error) {
    throw new Error(`Failed to create service ${name}: ${stderrOf(error)}`);
  }
}
...
async function removeService(name: string): Promise<void> {
  ...
  if (error) {
    ...
    console.log(`Removed legacy service ${name}.`);
  }
}
...
async function setVar(service: string, kv: string): Promise<void> {
  ...
  if (error) {
    throw new Error(`Failed to set ${key} on ${service}: ${stderrOf(error)}`);
  }
...
async function deleteVar(service: string, key: string): Promise<void> {
  ...
  if (error) {
    throw new Error(`Failed to delete ${key} on ${service}: ${stderrOf(error)}`);
  }
...
async function deployService(service: string): Promise<void> {
  ...
  if (error) {
    throw new Error(`Failed to start deploy for ${service}: ${stderrOf(error)}`);
  }
}
...
if (/^(1|true)$/i.test(Bun.env.DEPLOY_ONLY?.trim() ?? '')) {
  ...
  for (const service of services) {
    statuses.set(service, await waitForDeploy(service));
  }
  console.log('');
...
function requiredSuffixed(name: string, chainId: number): string {
  ...
  if (!value) {
    throw new Error(`Missing required env var: ${name}_${chainId}`);
  }
  return value;
}
...
for (const chain of chainSecrets) {
  ...
  for (const [key, value] of venueKeys) {
    if (value) {
      await setSecret(chain.service, key, value);
    } else if (existingKeys.has(key)) {
      await deleteVar(chain.service, key);
    }
  }
  if (betterstackHost) {
    await setVar(chain.service, `BETTERSTACK_INGESTING_HOST=${betterstackHost}`);
  }
  if (betterstackToken) {
    await setSecret(chain.service, 'BETTERSTACK_SOURCE_TOKEN', betterstackToken);
  }
...
for (const chain of chainSecrets) {
  botStatuses.set(chain.service, await waitForDeploy(chain.service));
}
...
for (const [service, status] of botStatuses) {
  console.log(`  ${service}: ${status}`);
}
console.log('');
...
console.log(
  '     discovers + logs liquidatable positions but skips every liquidation — rerun this'
);
console.log('     script with venue inputs once it should route.');
console.log(
  '  2. Each bot needs a funded key + a real RPC before it can broadcast; Robinhood also'
);
