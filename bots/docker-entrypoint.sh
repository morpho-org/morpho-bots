...
# run every TICK_INTERVAL_S seconds (default 2 ≈ Base block time — the stuck-tx / cooldown block
# math assumes roughly per-block ticks). `bash` (not `sh`) is required for PIPESTATUS: a plain pipe
# reports only the LAST stage's code and `pipefail` only the aggregate, but we need EACH stage's.
#
# Data plane vs. log plane: stdout carries JSON-Lines records (sense → opportunity, act → tx +
# outcome, queue → outcome). The queue's outcome lines are the tail of the pipe, so they land in the
# container's stdout logs; everything a human reads (sense.end/act.end/queue.end summaries, warnings,
# errors) goes to STDERR. This script's own lifecycle lines (loop.start/loop.fatal) are stderr too.
#
# Per-stage exit-code contract (each of sense/act/queue honors it independently; see
...
echo "{\"level\":\"info\",\"event\":\"loop.start\",\"bot\":\"$BOT\",\"intervalS\":\"${TICK_INTERVAL_S:-2}\",\"home\":\"$MORPHO_BOTS_HOME\"}" >&2
while true; do
  bun dist/main.js "$BOT" sense | bun dist/main.js "$BOT" act | bun dist/main.js "$BOT" queue
  codes=("${PIPESTATUS[@]}")
  if [[ " ${codes[*]} " == *" 2 "* ]]; then
