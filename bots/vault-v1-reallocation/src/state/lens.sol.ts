...
// params, accrued Blue state, the vault's position, its cap, and the IRM's rateAtTarget. There is no
// client-side accrual: `Morpho.accrueInterest` runs inside the simulation, so every read below is
// exactly what the market holds at that block. That makes the `lens` entrypoint state-changing
// (`accrueInterest` is `nonpayable`) — harmlessly, since the simulation's writes never leave the
// `eth_call`.
//
