...
Deposit legs stop at 99.99% of each market's supply cap so interest accrual between read and mined
execution can't push a leg over cap. Withdrawals are listed first and the last deposit leg is
...
| `VAULT_WHITELIST`                                         | yes      | —           | comma-separated vault addresses; must be non-empty |
...
loading, strategy-config resolution, revert decoding, and a dependency-injected tick. There is no
