...
/**
 * Simulates the real reallocation — `vault.multicall([deallocate…, allocate…])` from the allocator
 * EOA, byte-for-byte what gets broadcast. Any revert (role revoked, cap exceeded, insufficient
 * idle, market no longer enabled) means do not broadcast; the tick gates on `ok` only. No signer;
 * never sends.
 */
