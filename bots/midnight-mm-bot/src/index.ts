...
async function main() {
  ...
  const deployment = addresses[config.chainId]
  const midnight = deployment.midnight
  const mempool = deployment.midnightMempool
  const ratifier = deployment.ecrecoverRatifier
  ...
  if (config.rpcUrlFallback) transports.push(http(config.rpcUrlFallback))
  ...
  for (;;) {
    ...
    for (const key of published) {
      ...
      if (start < oldestLiveEpoch) published.delete(key)
    }
