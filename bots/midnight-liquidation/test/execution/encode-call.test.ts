...
function outerCalls(data: Hex): readonly Hex[] {
  ...
  if (top.functionName !== 'exec_606BaXt') {
    throw new Error('expected exec_606BaXt');
  }
...
function decodeLiquidate(data: Hex) {
  ...
  if (liquidateCall.functionName !== 'call_g0oyU7o') {
    throw new Error('expected call_g0oyU7o');
  }
  const liquidate = decodeFunctionData({ abi: MidnightAbi, data: liquidateCall.args[3] });
  if (liquidate.functionName !== 'liquidate') {
    throw new Error('expected liquidate');
  }
...
describe('encodeLiquidationExec', () => {
  ...
  it('builds a balance-bound queue: zero-first approve to spender, spliced swap, zero-first repay', () => {
    ...
    if (approveZero.functionName !== 'call_g0oyU7o') {
      throw new Error('expected call_g0oyU7o');
    }
    ...
    if (approveZeroInner.functionName !== 'approve') {
    ...
    if (repayApproveZero.functionName !== 'call_g0oyU7o') {
      throw new Error('expected call_g0oyU7o');
    }
...
describe('Market ABI/type drift guard', () => {
  ...
  it('TS Market/CollateralParams field order matches MidnightAbi.liquidate', () => {
    ...
    if (!marketArg?.components) {
      throw new Error('liquidate market arg not found in MidnightAbi');
    }
    ...
    if (!cpArg?.components) {
      throw new Error('collateralParams not found in MidnightAbi');
    }
