...
if (zeroxKey) {
  await setSecret(BOT_SERVICE, 'ZEROX_API_KEY', zeroxKey);
}
if (oneinchKey) {
  await setSecret(BOT_SERVICE, 'ONEINCH_API_KEY', oneinchKey);
}
if (lifiKey) {
  await setSecret(BOT_SERVICE, 'LIFI_API_KEY', lifiKey);
}
if (allowBadDebtOnly) {
  await setVar(BOT_SERVICE, 'ALLOW_BAD_DEBT_ONLY=true');
}
if (betterstackHost) {
  await setVar(BOT_SERVICE, `BETTERSTACK_INGESTING_HOST=${betterstackHost}`);
}
if (betterstackToken) {
  await setSecret(BOT_SERVICE, 'BETTERSTACK_SOURCE_TOKEN', betterstackToken);
}
