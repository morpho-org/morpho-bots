...
| `MAX_GAS_LIMIT`                              | `15000000`   | `3000000`    | The spend bound, tightened where fees are real.     |
...
`MAX_FEE_GWEI` is deliberately **not** per-chain (`300` on both). It is not a spend bound but the
headroom the bump ladder escalates into: the queue drops a tx — latching a nonce hole — once a bumped
`maxFeePerGas` would exceed it, so a tight ceiling shortens the ladder exactly when congestion makes
it matter. `test/config.test.ts` pins the congested basefee each chain still completes its full ladder
at, driving bot-kit's own fee functions rather than restating them.
