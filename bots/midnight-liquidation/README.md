...
- **execution-reverted** — the node rejected this position's own transaction with an on-chain
  ...
  clears an entry a lower-ranked sibling's quote or simulation failure had already armed, because a
  chain-declined send is the position's latest and best evidence. Post-maturity the LIF ramps over an
  hour, so a min-out shortfall says nothing about the next attempt; backing off sampled that ramp
...
execution-reverted sends on one position is tracked and reported as `send.revert_streak` (warn) once
it has run for more than 15 minutes, carrying the revert count, the streak duration, the last
selector, and whether that selector stayed constant across the streak. A constant selector over a
long streak points at a structural fault — an expired route deadline, malformed aggregator calldata,
