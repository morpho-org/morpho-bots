...
| `MARKETS_API_URL`                                         | no       | public              | Midnight markets endpoint used as the market whitelist. Defaults to the public Morpho markets API; validated as a URL at startup.                                                                                                                                                    |
| `MARKET_LISTING`                                          | no       | `listed`            | Which slice of the markets API the whitelist pulls: `listed` (app-trust-layer markets — production), `unlisted` (only unlisted markets), or `all`. `unlisted`/`all` widen beyond the trust layer and are for a thin-canary staging bot only, where the wallet caps the blast radius. |
