...
/**
 * A simple token bucket: `tokens` refills at `rps` per second up to `burst`. `take()` resolves once a
 * token is available, sleeping the shortfall otherwise. Pure but for the injected `now`/`sleep`, so it
 * is deterministically testable and trivially hoistable to `@repo/utils` later.
 */
