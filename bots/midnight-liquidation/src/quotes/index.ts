 ...
 * per-collateral venue, predicts the seized collateral, fetches ONE executable quote (Uniswap is
 * local; aggregators make a single API call), and sanity-checks an aggregator's quoted output against
 * the free oracle reference — rejecting a route worse than `maxRouteImpactBps` below it. Quote/route
 * failures return `failed` (the tick backs the position off); an unconfigured collateral returns
 * `no_config` (no API call, no backoff). Quotes are made ONLY for liquidatable positions, so API usage
 * is bounded by the (small) liquidatable set, never the full candidate universe.
 */
export function composeQuoting(deps: {
  ...
  return {
    async quoteFor(plan, out) {
      ...
      const entry = collateral && swapByCollateral.get(getAddress(collateral.token))
      if (!collateral || !entry) return { kind: 'no_config' }
