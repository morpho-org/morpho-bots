 ...
 * lens already performed on-chain (`valid` = the obligation exists; `gateAllows` = the liquidator
...
export function isLiquidatable(out: LensOut): boolean {
  return (
    ...
    out.hasDebt &&
    !out.locked &&
    (out.blockTimestamp > out.obligation.maturity || !out.healthy)
  )
}

/**
 * Maps a fresh {@link LensOut} into the sizing {@link PlanInput}. The market-config fields
 * (`maturity`, `rcfThreshold`) come from the returned `obligation`; everything else is the flat
...
export function planInputFromLens(out: LensOut): PlanInput {
  return {
    ...
    maturity: out.obligation.maturity,
    hasDebt: out.hasDebt,
    locked: out.locked,
    healthy: out.healthy,
    debt: out.debt,
    badDebt: out.badDebt,
    maxDebt: out.maxDebt,
    rcfThreshold: out.obligation.rcfThreshold,
