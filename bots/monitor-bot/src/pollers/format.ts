...
// Both formatters guard toISOString: an absurd timestamp from the API (RangeError on an invalid
// Date) must degrade to the raw number, not drop the alert.
...
function formatUtcDate(unixSeconds: number) {
  const { data } = tryCatch(() => {
    const iso = new Date(unixSeconds * 1000).toISOString()
