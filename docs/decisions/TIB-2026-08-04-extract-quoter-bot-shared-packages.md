# TIB-2026-08-04: Extract quoter-bot shared packages
...
The quoter bot
([TIB-2026-07-27](./TIB-2026-07-27-midnight-quoter-bot-bot.md)) accumulated several
general-purpose concerns inside `bots/quoter-bot`: a CLI output presenter buried in
...
Everything below lived inside `bots/quoter-bot`: the CLI presenter in
`src/infrastructure/cli/quoter-bot-logger.ts`, `createQuoterBotObservability` (which
...
`quoterBotCycleHasFailure`), per-adapter spread assertions
...
bot's `quoter-bot-logger.ts`. Deliberately independent of `@repo/bot-kit`'s
...
- `createBotObservability`: generalized from `createQuoterBotObservability`. Composes
...
- `cycleHasFailure` — replaces the three byte-identical copies (`bootstrapCycleHasFailure`,
  `ladderCycleHasFailure`, `quoterBotCycleHasFailure`).
...
- **`BASE_CHAIN_ID` deduped** to a single export in
  `bots/quoter-bot/src/config/config.utils.ts`.
...
- [TIB-2026-07-27-midnight-quoter-bot-bot](./TIB-2026-07-27-midnight-quoter-bot-bot.md) —
