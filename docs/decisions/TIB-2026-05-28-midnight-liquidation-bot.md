...
Three production defects in the tick, found by a BetterStack review of `bot.liquidation.midnight`
(source 2607569), changed the observability contract this TIB documents. This addendum records the
...
inputs and the derived numbers (`lif`, `effectiveDebt`, `cap`, `capEff`, `seizedAssets`) so the
