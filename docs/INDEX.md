...
- [Superintend](../.agents/skills/superintend/SKILL.md) — the satisficing self-check, and how TIBs, implementations, and PR-review responses get independently reviewed
...
| [@repo/ci-scripts](../packages/ci-scripts/)               | Release pipeline logic: `Releases <bot>` intent, approval gate, deploy manifest | [README](../packages/ci-scripts/README.md) |
...
- [TIB-2026-07-14: Pared-down Slack CI notifications](./decisions/TIB-2026-07-14-slack-ci-notifications.md) — `github-script` workflows posting PR notifications and release notes to Slack, deliberately trimmed from `prime-monorepo`'s `@repo/ci-scripts` system — implemented; PR notifications retired 2026-09-09 (addendum), release notes kept
...
- [TIB-2026-09-09: Release intent in the PR description, gated in the deploy workflow](./decisions/TIB-2026-09-09-release-intent-gated-deploys.md) — `Releases <bot>` in the PR body replaces `release-<bot>` labels; a `Gate` job in `deploy-production.yml` deploys a bot only if a non-author approval postdates the intent; tags become `<bot>-<PR#>`; `packages/ci-scripts/manifest.json` enumerates CI-deployable bots; records why per-bot release branches (needs an App or deploy key) and subset trees were not adopted — accepted
...
- [TIB-2026-09-10: Use separate quoter maker and KMS signer accounts](./decisions/TIB-2026-09-10-quoter-operator-maker-kms-signer.md) — uses an operator-held maker and a KMS signer with full Midnight authority — proposed
- [TIB-2026-08-12: Quoter-bot KMS signing policy middleware](./decisions/TIB-2026-08-12-quoter-bot-kms-signing-middleware.md) — the earlier design made KMS the funded maker and placed quote, ratification, remediation, and aggregate economic policy behind Lambda — **superseded by TIB-2026-09-10**
