...
const CONFIG = {
  ...
  signer: {
    ...
    signPreparedTransaction: async (transaction: TransactionSerializableEIP1559) =>
      (await ACCOUNT.signTransaction(transaction)) as TransactionSerializedEIP1559
  }
} as const
...
describe('createSender', () => {
  ...
  it('claims sequential nonces, and syncNonce re-reads the chain pending nonce over the cursor', async () => {
    ...
    const { send, syncNonce } = createSender(CONFIG)
    const req = {
  ...
  it('preserves deterministic signer rejection instead of wrapping it as a send retry', async () => {
    mockRpc({
      eth_chainId: `0x${base.id.toString(16)}`,
      eth_getTransactionCount: '0x5',
      eth_estimateGas: '0x5208',
    ...
    const sender = createSender({
      ...
      signer: {
        ...CONFIG.signer,
        signPreparedTransaction: () => Promise.reject(rejection)
      }
    })
    await expect(
      sender.send({
  ...
  it('getReceipt maps a found receipt to its status + block number', async () => {
    ...
    expect(await createSender(CONFIG).getReceipt(PROBE)).toEqual({
      status: 'success',
      blockNumber: 10n
    })
  })
  ...
  it('accepts an injected account and still broadcasts via eth_sendRawTransaction', async () => {
    ...
    const sender = createSender({
      ...
      signer: {
        address: account.address,
        signPreparedTransaction: async transaction =>
          (await account.signTransaction(transaction)) as TransactionSerializedEIP1559
      }
    })
    expect(sender.address).toBe(account.address)
