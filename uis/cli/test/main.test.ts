...
describe('morpho-bots exit codes', () => {
  ...
  it('exits 2 for an unknown command (usage error — wrappers must stop)', () => {
    expect(run(['bogus']).code).toBe(2)
  })

  it('exits 2 for a tick with no usable config, with a startup.error line', () => {
  ...
  it('init scaffolds the home dir and a second run keeps existing files', () => {
